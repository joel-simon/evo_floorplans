...
from floor_plans.reproduction import DefaultReproduction
from floor_plans.stagnation import DefaultStagnation
...
class Config(object):
    ...
    # file, and the floor_plans framework doesn't need to know about this section in any way. This
    ...
    # classes you need to give to floor_plans are shown in the config file.
    ...
    def load(self, filename):
        ...
        if not parameters.has_section('Types'):
            raise RuntimeError("'Types' section not found in floor_plans configuration file.")


        # Phenotype configuration
        # rooms = int(parameters.get('phenotype', 'rooms'))
        # self.rooms = self.rooms = ['room_%i'%i for i in range(rooms-1)]
        self.initial_empty = int(parameters.get('phenotype', 'initial_empty'))
        self.initial_bathrooms = int(parameters.get('phenotype', 'initial_bathrooms'))
        self.entrances = int(parameters.get('phenotype', 'entrances'))
        ...
        if 'partial' in self.initial_connection:
            ...
            self.connection_fraction = int(p)
